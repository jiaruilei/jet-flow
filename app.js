...
function setCoachOpen(open,moveFocus=true){
  coachPanel.hidden=!open;
  coachPanel.style.display=open ? 'flex' : 'none';
  coachFab.setAttribute('aria-expanded',String(open));
  coachFab.setAttribute('aria-controls','coachPanel');
  if(moveFocus) (open ? coachInput : coachFab).focus();
}
coachFab.addEventListener('click',()=>setCoachOpen(coachPanel.hidden));
coachClose.addEventListener('click',()=>setCoachOpen(false));
document.addEventListener('keydown',event=>{
  if(event.key==='Escape'&&!coachPanel.hidden){event.preventDefault();setCoachOpen(false);}
});
...
async function coachAsk(text){
  ...
  coachBusy=true;coachSend.disabled=true;
  coachBody.setAttribute('aria-busy','true');
  ...
  coachBusy=false;coachSend.disabled=false;
  coachBody.setAttribute('aria-busy','false');
}
function sanitizeMath(msg) {
  // 1) Remove triple-backtick code fences; MathJax intentionally skips code blocks.
  //    This keeps normal formatting but lets LaTeX content be parsed.
  msg = msg.replace(/```(?:\w+)?\s*([\s\S]*?)```/g, '$1');

  // 2) Optional: collapse multiple blank lines (prettier)
  msg = msg.replace(/\n{3,}/g, '\n\n');

  return msg;
}

function typesetNow(el,attempt=0) {
  // Wait until MathJax is ready, then typeset only this element
  if (window.MathJax && window._mjReady) {
    MathJax.typesetPromise([el]).catch(err => console.error('MathJax typeset error:', err));
  } else if(attempt<40 && byId('MathJax-script')) {
    // Keep readable plain text if the optional CDN does not become available.
    setTimeout(() => typesetNow(el,attempt+1), 250);
  }
}

function appendCoach(msg, isSelf) {
  const div = document.createElement('div');
  div.className = 'coach-msg' + (isSelf ? ' coach-self' : '');

  // IMPORTANT: sanitize first so LaTeX isn't trapped in code fences
  const cleaned = isSelf ? String(msg) : sanitizeMath(String(msg));
  ...
  // Now render any LaTeX in this new message
  typesetNow(div);
}
...
function coachReply(q){
  ...
  if(mode==='quiz' && !quiz.checked && (q.includes('solve') || q.includes('sign') || q.includes('positive') || q.includes('direction'))){
    return 'For downward deflection, the force on the vane is rightward (+x) and upward (+y). First find A = πD²/4, then ṁ = ρVA. Use Fₓ = ṁV(1 − cosθ) and Fᵧ = ṁVsinθ. Check your answer to reveal the numerical solution.';
  }

  if(q.includes('hint')) return `Use steady momentum CV (slides 17–21 & Example 6.4): ∑F = ṁ(v_out − v_in).
ṁ = ρ V A, A = πD²/4; on-fluid: Fx=ṁV(cosθ−1), Fy=−ṁVsinθ; on-vane is opposite.`;

  if(q.includes('explain')) return `CV momentum balance (steady): ∑F = Σ ṁ_out v_out − Σ ṁ_in v_in.
For a jet deflected by θ: Fx = ṁ V (cosθ − 1), Fy = − ṁ V sinθ (on fluid). Vane feels the opposite.`;

  if(q.includes('solve')) return worked();
  if(q.includes('sign')||q.includes('positive')||q.includes('direction'))
    return `Downward deflection: fluid loses +x and gains −y ⇒ Fx(on fluid) ≤ 0, Fy(on fluid) ≤ 0.
Vane feels opposite: |Fx|=${fmt(Math.abs(Fx_vane),3)} N to +x, |Fy|=${fmt(Math.abs(Fy_vane),3)} N to +y.`;

  if(q.includes('units')) return `SI units: ρ kg/m³, D m, V m/s ⇒ A m², ṁ kg/s, F newtons (N).`;
  if(q.includes('area')||q.includes('diam')||q.includes('π')) return `A = πD²/4. With D=${fmt(Dm,3)} m ⇒ A=${fmt(Ajet,6)} m².`;
  return worked();

  function worked(){
    if(mode==='quiz'&&!quiz.checked) return 'Start with A = πD²/4 using D in metres. Calculate ṁ = ρVA, then Fₓ = ṁV(1 − cosθ) and Fᵧ = ṁVsinθ. Enter both force magnitudes and check your answer to see the worked solution.';
    return `Current case: ρ=1000 kg/m³, D=${fmt(Dm,3)} m, V=${fmt(S.V,2)} m/s, θ=${fmt(S.theta,1)}°.
A=${fmt(Ajet,6)} m² ⇒ ṁ=${fmt(mdot,3)} kg/s.
On fluid: Fx=${fmt(Fx_fluid,3)} N, Fy=${fmt(Fy_fluid,3)} N.
On vane: |Fx|=${fmt(Math.abs(Fx_vane),3)} N (→ +x), |Fy|=${fmt(Math.abs(Fy_vane),3)} N (↑ +y), |F|=${fmt(Fmag,3)} N.`;
  }
}
...
Use steady control-volume momentum (Example 6.4): ṁ = ρ V A, A = πD²/4;
on-fluid: Fx = ṁ V (cosθ - 1), Fy = - ṁ V sinθ. Force on the VANE is the opposite.
...
ρ=1000 kg/m³, D=${Dm.toFixed(3)} m, V=${S.V.toFixed(2)} m/s, θ=${S.theta.toFixed(1)}°.
A=${Ajet.toExponential(6)} m², ṁ=${mdot.toFixed(3)} kg/s.`;
  ...
  const parsedTemp=Number.parseFloat(chatTemp.value);
  const temperature=Number.isFinite(parsedTemp) ? clamp(parsedTemp,0,2) : 0.2;
  chatTemp.value=temperature;
